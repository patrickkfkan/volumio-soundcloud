...
var _RootViewHandler_instances, _RootViewHandler_getMe, _RootViewHandler_getTopFeaturedTracks, _RootViewHandler_getSelections, _RootViewHandler_getListFromSelection;
...
class RootViewHandler extends BaseViewHandler_1.default {
    ...
    async browse() {
        const fetches = [
            ...
            __classPrivateFieldGet(this, _RootViewHandler_instances, "m", _RootViewHandler_getSelections).call(this, 'mixed'),
            __classPrivateFieldGet(this, _RootViewHandler_instances, "m", _RootViewHandler_getSelections).call(this, 'charts')
        ];
...
_RootViewHandler_instances = new WeakSet(), _RootViewHandler_getMe = async function _RootViewHandler_getMe() {
    ...
    if (myProfile?.id) {
        ...
        const feedItem = {
            service: 'soundcloud',
            type: 'item-no-menu',
            title: SoundCloudContext_1.default.getI18n('SOUNDCLOUD_FEED'),
            icon: 'fa fa-feed',
            uri: `${this.uri}/${ViewHelper_1.default.constructUriSegmentFromView(feedView)}`
        };
...
}, _RootViewHandler_getSelections = async function _RootViewHandler_getSelections(type) {
    try {
        const selections = await this.getModel(model_1.ModelType.Selection).getSelections({ type });
        const lists = selections.reduce((result, selection) => {
            if (selection.items.length > 0) {
                result.push(__classPrivateFieldGet(this, _RootViewHandler_instances, "m", _RootViewHandler_getListFromSelection).call(this, type, selection));
            }
            return result;
        }, []);
        return lists;
...
}, _RootViewHandler_getListFromSelection = function _RootViewHandler_getListFromSelection(type, selection) {
